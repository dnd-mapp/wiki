...
- Native CSS nesting is supported broadly, but there's no need to migrate off Sass's own `&` nesting in SCSS files — the native CSS Nesting spec is still an Editor's Draft, and native nesting has no equivalent to Sass's `&__element`/`&--modifier` string-concatenation.
