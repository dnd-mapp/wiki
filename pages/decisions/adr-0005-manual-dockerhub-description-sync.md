...
[`template-app-angular`](https://github.com/dnd-mapp/template-app-angular)'s `sync-description` CI job pushed `.docker/DOCKERHUB.md` to the Docker Hub repository description on every change, authenticated with the same `DH_READ_WRITE` access token used to push images (see the [token-scope table](/domain/docker/publishing#docker-hub-access-token-scope) in the Docker Publishing glossary). It failed with `403 Forbidden` on every run.
...
We chose not to store that password as a CI secret. Every other Docker Hub credential in this pipeline is a purpose-scoped access token (see the Docker Publishing glossary's [token-scope table](/domain/docker/publishing#docker-hub-access-token-scope)). A full account password would be a large increase in blast radius for one low-frequency, non-critical sync step. Instead, `.docker/DOCKERHUB.md`'s header now documents that changes must be pasted into the Docker Hub UI by hand.
