...
[`template-app-angular`](https://github.com/dnd-mapp/template-app-angular)'s `sync-description` CI job pushed `.docker/DOCKERHUB.md` to the Docker Hub repository description on every change, authenticated with the same `DH_READ_WRITE` access token used to push images (see [[Docker Hub access token scope]] in the Docker Publishing glossary). It failed with `403 Forbidden` on every run.
