...
Size-based (t-shirt scale): `--space-xs`, `--space-sm`, `--space-md`, `--space-lg`, `--space-xl`. Each is a thin, qualitative pass-through of the Primitive numeric scale — no purpose/usage encoded in the name (contrast with color, where relational context is load-bearing).
