...
- Docker images are published to Docker Hub under the org's `dndmapp` account, sourced from the `DH_USERNAME` secret rather than derived from the GitHub org name. See [Publish Docker images to Docker Hub instead of GHCR](/decisions/adr-0004-docker-hub-over-ghcr) for why, and [Docker Publishing](/domain/docker/publishing#docker-hub-access-token-scope) for the token-scoping conventions that go with it.
